...
class Render_Controller extends CI_Controller
{
	...
	protected function uploadFile($name, $path = null)
	{

			$config['upload_path']          = $path ?? $this->foto_folder;
			$config['allowed_types']        = '*';
			$config['file_name']            = md5(uniqid("gmail", true));
	...
	private function navigationHtml($navigation)
	{
		...
		foreach ($main_menu as $menu) {
			...
			$menu_nama = $menu['menu_nama'] != null ? $menu['menu_nama'] : $menu['menu_nama'];
			...
			foreach ($menu['sub_menu'] as $sub_menu) {
				...
				$sub_menu_nama = $sub_menu['menu_nama'] != null ? $sub_menu['menu_nama'] : $sub_menu['menu_nama'];
	...
	public function getMenuTitle()
	{
		...
		$return = $return != null ? ($return['menu_nama'] != null ? $return['menu_nama'] : $return['menu_nama']) : '';
		return $return;
	}
